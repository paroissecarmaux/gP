...
import { buildField, readFieldValue } from '../js/ui/forms.js';
...
import { formatDate } from '../js/utils/dates.js';
import { personLabel } from '../js/utils/formatting.js';
import { indexById, staticOptions } from '../js/utils/indexBy.js';
import {
  personRepository,
  volunteerRepository,
  clergyRepository,
...
} from '../js/services/people.js';
import { functionRepository, listFunctionsForPerson, assignFunction, endFunctionAssignment } from '../js/services/functions.js';
import { groupRepository, listGroupsForPerson } from '../js/services/groups.js';
import { listForOwner, addCoordonnee } from '../js/services/contacts.js';
...
export const peopleListPage = () =>
  new ListView({
    ...
    columns: [
      ...
      { label: 'Naissance', render: (row) => formatDate(row.birthDate) },
    ],
...
export class PersonFormPage extends Component {
  ...
  async onMount() {
    try {
      this.entity = this.props.id ? await personRepository.get(this.props.id) : null;
      ...
      await this.setupProfileFieldset('volunteer', await (this.entity ? getVolunteerProfile(this.entity.id) : null), [
        { name: 'skills', label: 'Compétences', type: 'text' },
        { name: 'availability', label: 'Disponibilités', type: 'text' },
      ]);
      await this.setupProfileFieldset('clergy', await (this.entity ? getClergyProfile(this.entity.id) : null), [
      ...
      await this.setupProfileFieldset('employee', await (this.entity ? getEmployeeProfile(this.entity.id) : null), [
        { name: 'position', label: 'Poste', type: 'text' },
        { name: 'contractType', label: 'Type de contrat', type: 'select', options: staticOptions(CONTRACT_TYPES) },
      ...
      reportError(error, { source: 'Personne' });
    }
  }
  ...
  async setupProfileFieldset(key, profile, subFields) {
    ...
    for (const field of subFields) {
      container.appendChild(await buildField(field, profile?.[field.name]));
    }
  ...
  async populateFunctionSelect() {
    ...
    this.functionsById = indexById(functions);
  ...
  async refreshCoordonnees() {
    const coordonnees = await listForOwner('personne', this.entity.id);
  ...
  async refreshFunctions() {
    const assignments = await listFunctionsForPerson(this.entity.id);
  ...
  async refreshGroups() {
    const memberships = await listGroupsForPerson(this.entity.id);
  ...
  async handleAddCoordonnee(event) {
    ...
    try {
      await addCoordonnee('personne', this.entity.id, data);
      form.reset();
      await this.refreshCoordonnees();
    } catch (error) {
      reportError(error, { source: 'Personne' });
  ...
  async handleAssignFunction(event) {
    ...
    try {
      await assignFunction(this.entity.id, functionId);
      await this.refreshFunctions();
    } catch (error) {
      reportError(error, { source: 'Personne' });
    }
  ...
  async handleEndFunction(assignmentId) {
    const ok = await confirmModal('Retirer cette fonction ?', { confirmLabel: 'Retirer', danger: true });
    if (!ok) return;
    try {
      await endFunctionAssignment(assignmentId);
      await this.refreshFunctions();
    } catch (error) {
      reportError(error, { source: 'Personne' });
    }
  ...
  async saveProfiles(personId, form) {
    const readProfile = async (key, repository) => {
      ...
      const values = {};
      for (const field of this[`${key}Fields`]) values[field.name] = readFieldValue(field, form);
      await setProfile(repository, personId, enabled, values);
    };

    await readProfile('volunteer', volunteerRepository);
    await readProfile('clergy', clergyRepository);
    await readProfile('employee', employeeRepository);
  }
  ...
  async handleSubmit(event) {
    ...
    const values = {};
    for (const field of CORE_FIELDS) values[field.name] = readFieldValue(field, form);

    if (!values.firstName || !values.lastName) {
      notify('Le prénom et le nom sont obligatoires.', { type: 'error' });
      return;
    }

    try {
      if (!this.entity) {
        const duplicates = await findPotentialDuplicates(values.firstName, values.lastName);
        if (duplicates.length > 0) {
          const proceed = await confirmModal(
            `Une personne du même nom existe déjà : ${duplicates.map(personLabel).join(', ')}. Enregistrer quand même ?`,
      ...
      const saved = this.entity ? await personRepository.update(this.entity.id, values) : await personRepository.create(values);
      await this.saveProfiles(saved.id, form);
      notify('Personne enregistrée.', { type: 'success' });
      this.entity = saved;
...
export const personFormPage = (params) => new PersonFormPage({ id: params?.id ?? null });
